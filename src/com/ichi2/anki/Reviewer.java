...
public class Reviewer extends Activity implements IButtonListener {
     ...
     */
    private static final String[] ABSOLUTE_CSS_UNITS = {
            "px;", "pt;", "in;", "cm;", "mm;", "pc;"
    };
     ...
     */
    private static final String[] RELATIVE_CSS_UNITS = {
            "%;", "em;"
    };
    ...
    /**
     * The answer in the compare to field for the current card if answer should
     * be given by learner. Null if the CardLayout in the deck says do not type
     * answer. See also Card.getComparedFieldAnswer().
     */
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_ZEEMOTE_BUTTON_C:
                if (DeckManager.getMainDeck().undoAvailable()) {
                    ...
                    DeckTask.launchDeckTask(DeckTask.TASK_TYPE_UNDO, mUpdateCardHandler, new DeckTask.TaskData(
                            UPDATE_CARD_SHOW_QUESTION, DeckManager.getMainDeck(), mCurrentCard.getId(), false));
                }
                break;
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        case MENU_REDO:
            DeckTask.launchDeckTask(DeckTask.TASK_TYPE_REDO, mUpdateCardHandler, new DeckTask.TaskData(
                    UPDATE_CARD_SHOW_QUESTION, DeckManager.getMainDeck(), mCurrentCard.getId(), false));
            return true;
        }
        return false;
        ...
        if (requestCode == EDIT_CURRENT_CARD) {
            ...
            if (resultCode == RESULT_OK || resultCode == RESULT_EDIT_CARD_RESET) {
                ...
                DeckTask.launchDeckTask(DeckTask.TASK_TYPE_UPDATE_FACT, mUpdateCardHandler, new DeckTask.TaskData(
                        showQuestion, DeckManager.getMainDeck(), mCurrentCard));
    ...
    private void showDeleteCardDialog() {
        ...
        builder.setMessage(String.format(res.getString(R.string.delete_card_message),
                Utils.stripHTML(mCurrentCard.getQuestion()), Utils.stripHTML(mCurrentCard.getAnswer())));
        builder.setPositiveButton(res.getString(R.string.yes), new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                setNextCardAnimation(false);
                DeckTask.launchDeckTask(DeckTask.TASK_TYPE_DELETE_CARD, mDismissCardHandler, new DeckTask.TaskData(0,
                        DeckManager.getMainDeck(), mCurrentCard));
            }
        });
    ...
    private void updateStatisticBars() {
        ...
        Deck deck = DeckManager.getMainDeck();
        Utils.updateProgressBars(this, mSessionProgressBar, deck.getSessionProgress(), mStatisticBarsMax,
                mStatisticBarsHeight, true, false);
        Utils.updateProgressBars(this, mSessionYesBar, deck.getProgress(false), mStatisticBarsMax,
                mStatisticBarsHeight, true);
    }
...
//        Log.i(AnkiDroidApp.TAG, "question: '" + question + "'");
        ...
        if (mSimpleInterface) {
            ...
            if (mSpeakText && Integer.valueOf(android.os.Build.VERSION.SDK) > 3) {
                ReadText.setLanguageInformation(
                        Model.getModel(DeckManager.getMainDeck(), mCurrentCard.getCardModelId(), false).getId(),
                        mCurrentCard.getCardModelId());
            }
        }
    ...
    private void displayCardAnswer() {
        ...
        if (mSimpleInterface) {
            ...
            if (isQuestionDisplayed()) {
                ...
                if (ques.length() == 0) {
                    ques = new SpannableString(getResources().getString(R.string.simple_interface_hint,
                            R.string.card_details_question));
                    ((SpannableString) ques).setSpan(new StyleSpan(Typeface.ITALIC), 0, mCardContent.length(),
            ...
            if (typeAnswer()) {
                ...
                if (mCurrentCard != null) {
                    ...
                    Matcher matcher = sSpanPattern.matcher(Utils.stripHTMLMedia(ArabicUtilities.reshapeSentence(
                            comparedFieldAnswer, true)));
    ...
    private void updateCard(String content) {
        ...
        if (mCurrentCard != null) {
            ...
            content = myModel.getCSSForFontColorSize(mCurrentCard.getCardModelId(), mDisplayFontSize, mNightMode,
                    nightBackground) + Model.invertColors(content, mNightMode);
...
//        Log.i(AnkiDroidApp.TAG, "Initial content card = \n" + content);
        ...
        if (isQuestionDisplayed()) {
            if (sDisplayAnswer && (questionStartsAt != -1)) {
                question = Sound.parseSounds(mBaseUrl, content.substring(0, questionStartsAt), mSpeakText,
                        MetaDB.LANGUAGES_QA_QUESTION);
                answer = Sound.parseSounds(mBaseUrl, content.substring(questionStartsAt, content.length()), mSpeakText,
                        MetaDB.LANGUAGES_QA_ANSWER);
            } else {
                question = Sound.parseSounds(mBaseUrl, content.substring(0, content.length() - 5), mSpeakText,
                        MetaDB.LANGUAGES_QA_QUESTION) + "<hr/>";
            }
        } else {
    ...
    private void myLog(String text) {
        Log.i(AnkiDroidApp.TAG, text);
    }

    public void fillFlashcard(boolean flip) {
        Log.i(AnkiDroidApp.TAG, "running fillFlashcard");

        if (!flip) {
            myLog("is not flip");
            // // Log.i(AnkiDroidApp.TAG, "base url = " + mBaseUrl);
            if (mSimpleInterface) {
                myLog("simpleinterface");
                mSimpleCard.setText(mCardContent);
            } else if (mRefreshWebview) {
                myLog("refresh webview");
                ...
                myLog("removing from view");
                mCardFrame.removeView(mCard);
                myLog("destroying");
                mCard.destroy();
                mCard = mNextCard;
                myLog("creating new webview");
                mNextCard = createWebView();
                myLog("setting visibility");
                mNextCard.setVisibility(View.GONE);
                myLog("adding view");
                mCardFrame.addView(mNextCard, 0);
                // hunt for input issue 720, like android issue 3341
    ...
    /**
     * Adds a div html tag around the contents to have an indication, where
     * answer/question is displayed
     * 
     * @param content
     * @param isAnswer if true then the class attribute is set to "answer",
     *        "question" otherwise.
     * @return
     */
    ...
    /**
     * Parses content in question and answer to see, whether someone has hard
     * coded the font size in a card layout. If this is so, then the font size
     * must be replaced with one corrected by the relative font size. If a
     * relative CSS unit measure is used (e.g. 'em'), then only hierarchy in
     * 'span' tag is taken into account.
     * 
     * @param content
     * @param percentage - the relative font size percentage defined in
     *        preferences
     * @return
     */
    ...
    /**
     * @return true if the AnkiDroid preference for writing answer is true and
     *         if the Anki Deck CardLayout specifies a field to query
     */
